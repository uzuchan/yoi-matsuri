...
## D-009 (2026-06-14) D-002「全アセット・プロシージャル」を部分緩和 — 主役オブジェクトに glTF/GLB メッシュ形状の使用を許可(ハイブリッド段階導入)

決定者: ユーザー(executive-producer / 最終決裁者)。根拠: MEETING_MINUTES.md §T3 / §3-2 / §4 D-1(全エージェント合意をユーザーが裁定)。

- **決定**: D-002「全アセット・プロシージャル」を**部分緩和**する。プロシージャルを引き続き既定(default)とした上で、**主役オブジェクトに限り外部 glTF/GLB のメッシュ形状(geometry)の使用を許可**する。境界条件は以下:
  1. **形状は glTF・色とマテリアルはコードで §2 パレットに正規化**。外部テクスチャ/画像は引き続き不可(世界観=パレット+光は不変)。色正規化アダプタ(議事録 TA B-5)を必ず介す
  2. 読み込みは three 同梱の `three/examples/jsm/loaders/GLTFLoader` を使用。**新規ランタイム依存ゼロ**(D-002 の three 採用範囲内)
  3. 既存 `WorldObject({object, dispose, update?})` 契約に載せる。`Promise<WorldObject>` でロードし、ロード完了までは**プロシージャルをプレースホルダ**にする(描画の穴を作らない)
  4. **段階導入。最初の対象は「店主」**(主役1体・F-004=結果画面で店主が暗い問題を造形で根治)、次に「金魚」。一括モデル化はしない
  5. **AI 生成/外部の生メッシュをそのまま投入しない**。Blender でリトポ/減面し、性能予算に収めてから導入する
  6. **性能予算は不変**(三角形 ≤50k・draw call ≤100・動的 PointLight ≤6・gzip 合計 ≤500KB・通常 50FPS 以上)。glTF はこの予算内に収まること。超過時は導入しない
  7. **当面の見送り(非スコープ)**: KTX2、Draco、全シーンのモデル化、PBR テクスチャパイプライン(将来必要時は meshopt 優先で別途決定)
  8. **音響は対象外**(D-004 のプロシージャル音響は不変。本緩和は視覚のみ)
  9. **各 glTF アセット追加ごとに art-director の世界観/パレット承認 + 性能ゲート実測を必須**とする(屋台追加と同じレビュー規律)
- **理由**: 議事録で全エージェントが「全面置換でなくハイブリッド段階導入」で合意。主役(店主→金魚)を造形で底上げすると没入と既存課題(F-004)を改善でき、形状のみ外部化し色は §2 正規化することで世界観を壊さない。新規依存ゼロで D-002/D-003/D-006 と整合。
- **代替案と却下理由**:
  - (a) 全面 glTF 置換 → 却下(世界観・性能・工数のリスク大。プロシージャルの軽さを失う)
  - (b) 現状維持=全プロシージャル → 却下(店主/金魚の造形上限と F-004 が残る)
  - (c) 外部テクスチャも許可 → 却下(パレット統一=美術の核が崩れる。色はコード正規化を堅持)
- **影響**: ART §1 を art-director が改定(外部画像/テクスチャ禁止は維持しつつ主役の glTF 形状を許可)。技術詳細(GLTFLoader 結線・色正規化アダプタ・WorldObject の Promise 化・性能計測)は**店主スパイク着手時に TECHNICAL_ARCHITECTURE.md へ追記**(本エントリは方針確定のみ。実装は別タスク・別承認)。新規依存なし。
