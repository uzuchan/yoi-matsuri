...
test('「遊んでいく」を選ぶと goldfish 未登録でも例外なく安全に approach へ戻る(AC5)', async ({
  page,
}) => {
  const errors: string[] = []
  page.on('console', (m) => {
  ...
  // 「遊んでいく」(先頭)を Enter 確定。goldfish 未登録 → 安全に approach へフォールバック。
  await page.keyboard.press('Enter')
  await page.waitForTimeout(200)
  await expect(page.locator(DIALOGUE)).toHaveCount(0)

  // 行き止まりなし: approach に戻り、再度会話を開ける。
  await page.keyboard.press('KeyE')
  await page.waitForTimeout(150)
  await expect(page.locator(DIALOGUE)).toBeVisible()
...
test('マウスで「遊んでいく」を選ぶと goldfish 未登録でも安全に approach へ戻り再操作できる(AC5/AC4)', async ({
  page,
}) => {
  const errors: string[] = []
  page.on('console', (m) => {
  ...
  // 行き止まりなし: approach に戻り、E で会話を開き直せる(dialogue シーンに取り残されていない)。
  await page.keyboard.press('KeyE')
  await page.waitForTimeout(200)
  await expect(page.locator(DIALOGUE)).toBeVisible()
